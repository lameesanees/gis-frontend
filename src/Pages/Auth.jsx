...
import { registerAPI, verifyOTPAPI, loginAPI } from "../Services/allAPI"; // Adjust the import path as needed
...
function Auth({ register }) {
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    if (!userData.username || !userData.email || !userData.password || !userData.aadhaar) {
      Swal.fire({
        title: "Warning!",
  ...
  const handleOTPVerification = async (e) => {
    e.preventDefault();
    try {
      const response = await verifyOTPAPI({ email: userData.email, otp: userData.otp });
      if (response.status === 200) {
        ...
        navigate("/register"); // Redirect back to the register page if OTP is invalid
      }
    } catch (error) {
